    ...
    <div class="container mx-auto sm:flex sm:items-center sm:justify-between">
      <div class="flex justify-between px-4 py-3">
        ...
        <div class="flex sm:hidden">
          <button @click="toggle" type="button" class="px-2 text-gray-500 hover: focus:outline-none focus:">
            <svg class="h-6 w-6 fill-current hover:text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <div class="sm:flex sm:items-center">
          ...
          <div class="relative px-5 py-5 sm:py-0 sm:ml-4 sm:px-0">
            <div class="flex items-center sm:hidden mb-5">
              ...
              <span v-if="currentUser$" class="ml-4 font-semibold sm:hidden">{{currentUser$.displayName}}</span>
            </div>
            <div class="sm:hidden">
              <a v-if="!currentUser$" @click="signInWithGoogle" class="block hover:text-pink-600">Sign in with Google</a>
              <a v-if="currentUser$" href="#account" class="block hover:text-pink-600">Account settings</a>
              <a v-if="currentUser$" @click="signOut" class="mt-3 block hover:text-pink-600">Sign out</a>
            </div>
            <Dropdown class="hidden sm:block">
              <template #trigger="{ hasFocus, isOpen }">
                <span class="block h-8 w-8 overflow-hidden rounded-full border-2" :class="[(hasFocus || isOpen) ? 'border-white' : 'border-pink-800']">
                  <svg v-if="!currentUser$" class="w-full h-full  fill-current" viewBox="0 0 20 20"><path d="M5 5a5 5 0 0 1 10 0v2A5 5 0 0 1 5 7V5zM0 16.68A19.9 19.9 0 0 1 10 14c3.64 0 7.06.97 10 2.68V20H0v-3.32z"/></svg>
